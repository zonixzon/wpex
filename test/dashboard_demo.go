...
func runDashboardDemo() {
	...
	// Verifica prima se il server è disponibile
	fmt.Println("\n🔍 Checking if WPEX server is running on :8080...")
	...
	if err != nil {
		fmt.Println("❌ WPEX server is not running on :8080")
		fmt.Println("💡 To start WPEX with stats server:")
		fmt.Println("   ./wpex.exe --stats :8080 <your-config>")
		fmt.Println("\n🌐 Expected endpoints when server is running:")
		fmt.Println("   Main Dashboard: http://localhost:8080/")
		fmt.Println("   JSON API:       http://localhost:8080/stats")
		fmt.Println("   Health Check:   http://localhost:8080/health")
		fmt.Println("\n⚠️ Test skipped - server not available")
		return
	}

	fmt.Println("✅ WPEX server is running!")
